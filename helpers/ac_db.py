...
class Tools:
    ...
    def generate_device(self, devtype, host, mac, name=None, cloud=None, update_interval=0):
        if devtype == 0x4E2a:
            return AcDb(host=host, mac=mac, name=name, cloud=cloud, devtype=devtype, update_interval=0)
        else:
            return Device(host=host, mac=mac, devtype=devtype, update_interval=update_interval)

    def discover(self, timeout=None, local_ip_address=None):
        ...
        if timeout is None:
            ...
            cs.close()
            return self.generate_device(devtype, host, mac, name=name, cloud=cloud)
        else:
            while (time.time() - starttime) < timeout:
                ...
                cloud = bool(responsepacket[-1])
                dev = self.generate_device(devtype, host, mac, name=name, cloud=cloud)
                devices.append(dev)
